...
class _ProjectsPageState extends State<ProjectsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final highlightedProject = AppConstants.projects.firstWhere((p) => p["id"] == "journaling");
    final otherProjects = filteredProjects.where((p) => p["id"] != "journaling").toList();
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              if (_selectedFilter == "All" || (highlightedProject["tech"] as List<String>).contains(_selectedFilter)) ...[
                _buildHighlightedBanner(context, highlightedProject, isDark, isMobile),
                const SizedBox(height: 40),
              ],

              // Others Grid
              otherProjects.isEmpty && (_selectedFilter != "All" && !(highlightedProject["tech"] as List<String>).contains(_selectedFilter))
                  ? const SizedBox(
                      height: 200,
                      child: Center(child: Text("No other projects match this filter.")),
                    )
                  : LayoutBuilder(
                  ...
                  ElevatedButton(
                    ...
                    child: const Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 10),
                        Text("Explore Editor & Architecture Details", style: TextStyle(fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                ],
  ...
  // Pop-up window displaying structural details & the interactive WYSIWYG editor mock
  void _openEditorDemoDialog(BuildContext context, Map<String, dynamic> project) {
    showDialog(
      context: context,
      builder: (context) {
        return Dialog(
          ...
          child: ConstrainedBox(
            ...
            child: InteractiveEditorShowcase(project: project),
          ),
        );
      },
    );
...
class InteractiveEditorShowcase extends StatefulWidget {
  ...
  @override
  State<InteractiveEditorShowcase> createState() => _InteractiveEditorShowcaseState();
}

class _InteractiveEditorShowcaseState extends State<InteractiveEditorShowcase> {
  final TextEditingController _editorController = TextEditingController(
    text: "Welcome to my digital journaling environment! This interactive panel represents a custom WYSIWYG markdown block parser built on Flutter.\n\nHighlight text and toggle headers or styles to test the reactive UI!"
  );
          ...
          Expanded(
            child: ListView(
              children: [
                ...
                const SizedBox(height: 4),
                Text("Simulate the formatting toolbar features below:", style: theme.textTheme.bodySmall),
                const SizedBox(height: 12),
                      ...
                      Row(
                        children: [
                          Container(
                            ...
                            width: 6,
                            decoration: BoxDecoration(
                              color: _editorStatus.contains("queue") ? Colors.green : Colors.amber,
                              shape: BoxShape.circle,
                            ),
                          ),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border(
                      left: BorderSide(color: isDark ? AppConstants.borderDark : AppConstants.borderLight),
                      right: BorderSide(color: isDark ? AppConstants.borderDark : AppConstants.borderLight),
                      bottom: BorderSide(color: isDark ? AppConstants.borderDark : AppConstants.borderLight),
                    ),
                  ),
                  child: TextField(
  ...
  Widget _toolbarToggleButton({
    ...
    return Tooltip(
      message: tooltip,
      child: IconButton(
        icon: Icon(icon, color: isActive ? theme.primaryColor : theme.iconTheme.color?.withOpacity(0.6)),
        onPressed: onPressed,
